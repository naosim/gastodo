...
function (_super) {
  ...
  ScheduledStartDateTime.prototype.toGasValue = function () {
    var d = this.value.getFullYear() + "/" + zerofill(this.value.getMonth() + 1) + "/" + zerofill(this.value.getDate());
...
/** @class */
function () {
  function PendingTask(taskId, taskTitle, scheduledStartDateTime, execTiming, estimateHour) {
    this.taskId = taskId;
    this.taskTitle = taskTitle;
  ...
  PendingTask.prototype.toGasValue = function () {
...
/** @class */
function (_super) {
  __extends(TextArg, _super);

  function TextArg() {
    return _super !== null && _super.apply(this, arguments) || this;
  }

  TextArg.prototype.getType = function () {
    if (parseInt(this.value) == NaN) {
      if (this.value == ExecTiming.AM || this.value == ExecTiming.PM1 || this.value == ExecTiming.PM2 || this.value == ExecTiming.OVER) {
...
(function (TextArgType) {
  TextArgType[TextArgType["schedule"] = 0] = "schedule";
  TextArgType[TextArgType["execTiming"] = 1] = "execTiming";
  TextArgType[TextArgType["estimate"] = 2] = "estimate";
  TextArgType[TextArgType["text"] = 3] = "text";
})(TextArgType || (TextArgType = {})); //execTiming: ExecTiming


function createPendingTask(text) {
  var today = getToday();
  text = text.trim();
  var args = text.split(' ').map(function (v) {
    return new TextArg(v);
  });
  var scheduledStartDateTime = new ScheduledStartDateTime(today);
  var estimateHour;
  var execTiming = ExecTiming.AM;
  var startIndex = -1; // 1要素目が見積もり

  var firstType = args[0].getType();
  ...
  if (firstType == TextArgType.estimate) {
    ...
    if (firstType == TextArgType.execTiming) {
      ...
      var d = new Date(today.getFullYear() + "-" + (today.getMonth() + 1) + "-" + today.getDate() + " " + args[0].value.slice(0, 2) + ":" + args[0].value.slice(2));
  ...
  var title = args.slice(startIndex).map(function (v) {
    return v.value;
  }).join(' ');
  return new PendingTask(new TaskId("T" + Date.now()), new TaskTitle(title), scheduledStartDateTime, execTiming, estimateHour);
}
...
document.querySelector('#sendTodaysTaskButton').addEventListener('click', function () {
  var v = document.querySelector('#todaysTaskInput');
  var task = createPendingTask(v.value);
  console.log(task.toGasValue().join('\t'));
  sheet.add(task, function () {});
});
},{"./vo":"0pHl","./option":"Vmx1"}]},{},["jP6t"], null)
//# sourceMappingURL=/main.7da74d5d.map